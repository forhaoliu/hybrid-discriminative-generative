...
def simple_separated_format(separator):
    ...
    """
    return TableFormat(
        None, None, None, None, headerrow=DataRow("", separator, ""), datarow=DataRow("", separator, ""), padding=0, with_header_hide=None
    )
...
def _normalize_tabular_data(tabular_data, headers):
    ...
    * pandas.DataFrame (usually used with headers="keys")
    ...
    if hasattr(tabular_data, "keys") and hasattr(tabular_data, "values"):
        ...
        elif headers == "keys" and len(rows) > 0 and isinstance(rows[0], tuple) and hasattr(rows[0], "_fields"):  # namedtuple
...
def setup_logger(
    ...
    **create_log_dir_kwargs
    ...
    text_log_path = osp.join(tempdirpath, text_log_file)
